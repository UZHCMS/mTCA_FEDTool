...
void Amc13Interface::ConfigureAmc13WithInput(int mode, int rate, int burst)
{
    ...
    if (fDescription->fTrigger != nullptr)
    {
        fAMC13->configureLocalL1A (fDescription->fTrigger->fLocal, mode, burst, rate, fDescription->fTrigger->fRules );

        //Edit GA: not sure if this is actually required
        fAMC13->write (amc13::AMC13Simple::T1, "CONF.LOCAL_TRIG.FAKE_DATA_ENABLE", 1);

        std::cout << "Configuring local L1A: Mode: " << mode << " Rate: " << rate << " Burst: " << burst << " Rules: " << fDescription->fTrigger->fRules << std::endl;
    }
