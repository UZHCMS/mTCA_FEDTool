...
class PixFEDFWInterface : public RegManager
{
  ...
  public:
    ...
    */
    ...
    std::vector<uint32_t> readSpyFIFO(std::vector<uint32_t>& cSpyA, std::vector<uint32_t>& cSpyB);
    /*!
     * \brief: read the contents of FIFO 1 for TBM cores A & B
     */
    std::string readFIFO1();
    std::string readFIFO1(std::vector<uint32_t>& cFifo1A, std::vector<uint32_t>& cFifo1B, std::vector<uint32_t>& cMarkerA, std::vector<uint32_t>& cMarkerB);
    /*
