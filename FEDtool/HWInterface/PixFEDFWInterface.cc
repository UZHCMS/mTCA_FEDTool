...
std::vector<uint32_t> PixFEDFWInterface::readSpyFIFO(std::vector<uint32_t> & cSpyA, std::vector<uint32_t> & cSpyB)
{
  // std::vector<uint32_t> cSpyA;
  // std::vector<uint32_t> cSpyB;
...
std::string PixFEDFWInterface::readFIFO1(std::vector<uint32_t>& cFifo1A, std::vector<uint32_t>& cFifo1B, std::vector<uint32_t>& cMarkerA, std::vector<uint32_t>& cMarkerB)
{
    std::stringstream cFIFO1Str;
    //std::vector<uint32_t> cFifo1A;
    //std::vector<uint32_t> cFifo1B;
...
void PixFEDFWInterface::Start()
{
    ...
    fNthAcq = 0;
    WriteReg ( "pixfed_ctrl_regs.PC_CONFIG_OK", 1 );
    std::cout << "PC_CONFIG_OK (1) in Start" << std::endl;          // to avoid huge printouts disabled by Benno 
    readTTSState();
}
...
void PixFEDFWInterface::Stop()
{
    //Stop the DAQ
    WriteReg ( "pixfed_ctrl_regs.PC_CONFIG_OK", 0 );
    std::cout << "PC_CONFIG_OK (0) in Stop" << std::endl;           // to avoid huge printouts disabled by Benno 
    readTTSState();
}
