...
class PixFEDInterface
{
  ...
  public:
    ...
    */
    ...
    std::vector<uint32_t> readSpyFIFO ( const PixFED* pFED, std::vector<uint32_t> & cSpyA, std::vector<uint32_t> & cSpyB);
    /*!
     * \brief: read contents of FIFO1
     * \return string of the FIFO1 console dump
     */
    std::string readFIFO1 ( const PixFED* pFED );
    std::string readFIFO1 (  const PixFED* pFED, std::vector<uint32_t>& cFifo1A, std::vector<uint32_t>& cFifo1B, std::vector<uint32_t>& cMarkerA, std::vector<uint32_t>& cMarkerB);
    /*!
