...
std::vector<uint32_t> PixFEDInterface::readSpyFIFO ( const PixFED* pFED, std::vector<uint32_t> & cSpyA, std::vector<uint32_t> & cSpyB)
{
    setBoard ( pFED->getBeId() );
    return fFEDFW->readSpyFIFO(cSpyA, cSpyB);
}
...
std::string PixFEDInterface::readFIFO1 ( const PixFED* pFED, std::vector<uint32_t>& cFifo1A, std::vector<uint32_t>& cFifo1B, std::vector<uint32_t>& cMarkerA, std::vector<uint32_t>& cMarkerB)
{
    setBoard ( pFED->getBeId() );
    return fFEDFW->readFIFO1(cFifo1A, cFifo1B, cMarkerA, cMarkerB);
}
